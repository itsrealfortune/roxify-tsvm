...
import type { ShellModule } from "typescript-virtual-container";

/**
 * Search for a regex pattern in files or stdin with common flags.
 * @category misc
 * @params [encode|decode|list|havepassphrase] ["<file>..."]
 */
export const roxifyCommand: ShellModule = {
    name: "roxify",
    description: "ROX CLI — Encode/decode binary in PNG or WAV",
    category: "misc",
    params: ["[encode|decode|list|havepassphrase] [<file>...]"],
    run: async ({ args, cwd, shell }) => {
        const help = `
